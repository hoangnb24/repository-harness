...
grep -Fq 'workflow_dispatch:' "$workflow" || fail "diagnostic workflow is not manually dispatchable"
grep -Fq 'refs/heads/refactor/harness-v1' "$workflow" || fail "diagnostic workflow is not scoped to refactor/harness-v1"
! grep -Fq -- '- .github/harness-v1-diagnostic-request' "$workflow" || fail "retired sentinel still triggers the workflow"
! grep -Fq 'push:' "$workflow" || fail "diagnostic workflow still runs from sentinel pushes"
...
grep -Fq 'test "$EVENT_NAME" = workflow_dispatch' "$workflow" || fail "event identity check is missing"
grep -Fq 'test "$DISPATCH_REF" = refs/heads/refactor/harness-v1' "$workflow" || fail "dispatch ref identity check is missing"
...
grep -Fq 'test "$CANDIDATE_SHA" = "$WORKFLOW_REVISION"' "$workflow" ||
  fail "dispatch candidate is not identical to the executing workflow revision"
