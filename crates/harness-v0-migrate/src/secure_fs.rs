...
impl SecureRoot {
    pub fn open(path: &Path) -> Result<Self> {
        #[cfg(not(unix))]
        {
            ...
            Err(BridgeError::Unsupported(
                "descriptor-relative repository capture is unavailable on Windows until Phase 7"
                    .into(),
            ))
        }
        #[cfg(unix)]
    ...
    pub fn open_dir_fd(
        ...
        parent.ok_or_else(|| BridgeError::Usage("empty directory path".into()))
    }

    #[cfg(unix)]
    pub fn create_dir_exact(&self, relative: &str) -> Result<()> {
    ...
    #[cfg(unix)]
    fn open_parent(&self, relative: &str, create: bool) -> Result<(std::os::fd::OwnedFd, String)> {
        ...
        let descriptor = if parent.is_empty() {
            ...
            self.open_dir_fd(parent, create, parent.starts_with(".harness-v0-archive"))?
        };
    ...
    pub fn read_private(&self, relative: &str, expected_len: Option<usize>) -> Result<Vec<u8>> {
        #[cfg(unix)]
        {
            use rustix::fs::fstat;
            let descriptor = self.open_required_regular(relative)?;
            let stat = fstat(&descriptor).map_err(|error| map_errno(relative, error))?;
            if stat.st_mode as u32 & 0o777 != 0o600 || stat.st_uid != self.stat.st_uid {
                return Err(BridgeError::Conflict(format!(
                    "private custody file has invalid owner or mode: {relative}"
                )));
            }
            let mut file = File::from(descriptor);
            let mut bytes = Vec::new();
            ...
            if expected_len.is_some_and(|expected| bytes.len() != expected) {
                return Err(BridgeError::Conflict(format!(
                    "private custody file has invalid length: {relative}"
                )));
            }
            Ok(bytes)
        }
    ...
    pub fn preflight_new_output(&self, relative: &str) -> Result<()> {
        validate_relative(relative)?;
        if relative == ".harness"
            || relative.starts_with(".harness/")
            || relative == ".harness-v0-archive"
            || relative.starts_with(".harness-v0-archive/")
        {
            return Err(BridgeError::Usage(
                "export output must be outside V1 and archive custody".into(),
            ));
        }
        if self.read_optional(relative)?.is_some() {
            return Err(BridgeError::Conflict(format!(
    ...
    pub fn rename_no_replace(&self, source: &str, destination: &str) -> Result<()> {
        ...
        #[cfg(not(any(target_os = "linux", target_os = "macos")))]
        {
            ...
            Err(BridgeError::Unsupported(
                "atomic descriptor-relative no-replace publication is unavailable until Phase 7"
                    .into(),
            ))
        }
    }
    ...
    pub fn directory_exists(&self, relative: &str) -> Result<bool> {
        #[cfg(unix)]
        match self.open_dir_fd(relative, false, false) {
            Ok(_) => Ok(true),
    ...
    pub fn list_names(&self, relative: &str) -> Result<Vec<String>> {
        #[cfg(unix)]
        {
            let directory = self.open_dir_fd(relative, false, false)?;
            self.list_names_fd(&directory, relative)
        }
        #[cfg(not(unix))]
        unreachable!("non-Unix roots are never constructed")
    }
...
fn validate_relative(value: &str) -> Result<()> {
    harness_core::path::validate_repository_relative(value)
        .map(|_| ())
        .map_err(|_| BridgeError::Usage("path must be safe and repository-relative".into()))
}
